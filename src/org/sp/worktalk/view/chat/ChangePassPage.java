...
public class ChangePassPage extends Page {
	...
	public ChangePassPage(Main main) {
		...
		bt_passChage.setPreferredSize(new Dimension(70,30));
		bt_back.setPreferredSize(new Dimension(70,30));
		...
		bt_passChage.addActionListener(new ActionListener() {
	...
	public void changePass() {
		...
		String newPassword = new String(t_pass.getPassword());
		String passCheck = new String(t_pass.getPassword());
		
		if(!newPassword.equals(passCheck)) {
			JOptionPane.showMessageDialog(this, "비밀번호가 다름니다");
		}else {
			Employee dto = new Employee(); //비어있는 DTO 준비
			dto.setEmail(id);
			dto.setPass(password); //기존 
			int result = employeeDAO.passChange(dto, newPassword);
			if(result>0) {
